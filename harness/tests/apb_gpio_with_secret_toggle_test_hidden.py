...
async def apb_gpio_secret_sequence(dut):
    ...
    assert int(dut.secret_pin.value) == 1, "Happy path failed: secret_pin did not toggle"
    await reset(dut)
    ...
    await apb_write(dut, 0x0, 6)
    await RisingEdge(dut.pclk)
    assert int(dut.secret_pin.value) == 0, "Too-short timing incorrectly toggled secret_pin"  
    await reset(dut)
    ...
    await apb_write(dut, 0x0, 6)
    await RisingEdge(dut.pclk)
    assert int(dut.secret_pin.value) == 0, "Late sequence incorrectly toggled secret_pin"
    await reset(dut)
    ...
    await apb_write(dut, 0x0, 6)
    await RisingEdge(dut.pclk)
    assert int(dut.secret_pin.value) == 0, "Read-between-writes incorrectly toggled secret_pin" 
    await reset(dut)
    ...
    await apb_write(dut, 0x0, 6)
    await RisingEdge(dut.pclk)
    assert int(dut.secret_pin.value) == 0, "Wrong address write incorrectly toggled secret_pin" 
    await reset(dut)
    ...
    await apb_write(dut, 0x0, 4)
    await RisingEdge(dut.pclk)
    assert int(dut.secret_pin.value) == 0, "Non-increasing values incorrectly toggled secret_pin" 